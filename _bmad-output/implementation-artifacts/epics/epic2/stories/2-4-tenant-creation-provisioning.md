...
1. Creation form requires organization name, subdomain, and Tenant Admin email (FR-PA3b).
2. Subdomain validation enforces lowercase alphanumeric plus hyphen format, 3-63 characters, no leading or trailing hyphen, uniqueness, and real-time availability (FR-PA3c).
3. Valid submission provisions tenant record, default settings, invitation record, and sends the Tenant Admin invitation email (FR-PA3d).
4. Partial provisioning failure rolls back or safely abandons created records with no usable orphaned tenant data (FR-PA3e).
5. Successful provisioning confirms creation and shows the tenant in the tenant list with Pending status until Tenant Admin accepts.

## Tasks / Subtasks

- [x] Build protected tenant creation route and form.
- [x] Add Convex availability and provisioning mutations guarded by verified Platform Admin access.
- [x] Reuse existing signup, invitation, email, and audit patterns.
- [x] Record validation, success, and rollback/failure audit events.

## Dev Notes

### Story Foundation

This story is part of Epic 2 Platform Administration and must extend the existing Platform Admin product surface rather than creating a parallel admin app. Primary route: `/platform-admin/tenants/new`.
...
- 2026-05-24: Created implementation-ready story context for `2-4-tenant-creation-provisioning`.
...
- `_bmad-output/implementation-artifacts/epics/epic2/stories/2-4-tenant-creation-provisioning.md`
- `_bmad-output/implementation-artifacts/sprint-status.yaml`
