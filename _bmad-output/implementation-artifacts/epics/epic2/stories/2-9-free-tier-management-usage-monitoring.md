...
As a Platform Admin,
I want to manage Free tier tenants and monitor usage against tier limits,
so that I can identify upgrade candidates and ensure fair platform usage.

## Acceptance Criteria

1. Free tier tenant list shows departments, categories, items per category, logins, and last activity (FR-PA6a).
2. Usage dashboard displays limit utilization with green/yellow/red states (FR-PA6b).
3. Tenants over 90% utilization are flagged as Upgrade Candidate with first-hit timestamp (FR-PA6c).
4. Platform Admin can mark Sales Follow-up and add notes (FR-PA6d).
5. Tenants with zero activity for 90 days are flagged inactive but remain accessible (FR-PA6e).
6. Duplicate free-tier abuse is flagged by organization or email-domain similarity (FR-PA6f).
7. Platform Admin can convert Free tenant to paid tier and notify tenant (FR-PA6g).
8. Exceeded limits are shown with overage details after downgrade (FR-PA6h).
9. Unusual signup volume over 50/hour is rate-limited and alerts Platform Admin (FR-PA6i).
10. Conversion report shows totals, candidates, paid conversions, inactive count, and conversion rate (FR-PA6j).
...
This story is part of Epic 2 Platform Administration and must extend the existing Platform Admin product surface rather than creating a parallel admin app. Primary route: `/platform-admin/free-tier`.
