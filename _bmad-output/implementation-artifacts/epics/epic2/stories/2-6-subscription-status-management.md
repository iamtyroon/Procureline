...
1. Subscription list shows all tenants with filters for tier, status, payment method, and date range (FR12, FR-PA4a).
2. Revenue reports show MRR, ARR, and revenue by tier, with CSV export (FR-PA4b).
3. Subscription detail shows tenant, tier, status, next billing date, amount, and payment method.
