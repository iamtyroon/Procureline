...
export function runTenantAdminInstitutionalVisibilityTests(): string[] {
    const completedTests: string[] = [];
    const now = Date.UTC(2026, 6, 20, 10, 0, 0);

    const overview = buildTenantAdminInstitutionalOverview({
        auditLogs: [
            {
    ...
    assert.deepEqual(
        overview.rows.map((row) => row.departmentId),
        ["department-finance", "department-finance-duplicate", "department-hr"],
    );
    assert.equal(overview.rows[0]?.planId, "plan-approved");
    assert.equal(overview.rows[0]?.status, "approved");
    assert.equal(overview.rows[0]?.budget.utilizationPercent, 120);
    assert.equal(overview.rows[0]?.duContacts.length, 1);
    assert.equal(overview.rows[1]?.status, "not_started");
    assert.equal(overview.rows[2]?.procurementOfficer.state, "inactive");
    assert.equal(overview.summary.totalDepartments, 3);
    assert.equal(overview.summary.totalAllocated, 1400);
    assert.equal(overview.summary.totalUtilized, 1200);
    assert.equal(overview.summary.anomalyCount >= 5, true);
    assert.equal(
        overview.anomalies.some((anomaly) => anomaly.type === "duplicate_department_code"),
        true,
    );
    assert.equal(
        overview.anomalies.some((anomaly) => anomaly.type === "invalid_budget_allocation"),
        true,
    );
    completedTests.push(
        "tenant-admin institutional overview shapes every active department once, reuses canonical plan priority, deduplicates safe DU contacts, and reports budget anomalies",
    );

    const filteredRows = filterInstitutionalOverviewRows(overview.rows, {
    ...
    const filteredAnomalies = overview.anomalies.filter((anomaly) =>
        filteredDepartmentIds.has(anomaly.departmentId),
    );
    const filteredSummary = summarizeInstitutionalOverview(
        filteredRows,
        filteredAnomalies,
    );
    assert.equal(filteredSummary.totalDepartments, 1);
    assert.equal(filteredSummary.approvedOrSubmittedLabel, "1 of 1");
    assert.equal(overview.poRollups[0]?.status, "attention_needed");
    assert.equal(overview.availableFiscalYears.includes("2025-2026"), true);
    completedTests.push(
        "tenant-admin institutional filters stay aligned with the server-shaped dataset and fiscal years include plan, snapshot, deadline, and audit signals",
    );
    ...
    assert.equal(exportPreview.metadata.requestId, "export-1");
    assert.equal(exportPreview.metadata.asOf, now);
    assert.equal(exportPreview.metadata.fiscalYear, "2026-2027");
    assert.equal(exportPreview.departments[0]?.plan?.workspaceState, undefined);
    assert.equal(exportPreview.departments[0]?.plan?.internalReviewComments, undefined);
    assert.equal(exportPreview.departments[0]?.duContacts.length, 1);
    completedTests.push(
        "tenant-admin institutional export previews are point-in-time, tenant-scoped, and omit sensitive workflow internals",
    );
