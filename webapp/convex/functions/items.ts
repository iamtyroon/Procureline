    ...
    handler: async (ctx) => {
        ...
        return {
            ...
            meta: {
                ...
                importLimit: buildProcurementItemImportLimitState({ tier: tenant.tier }),
                tier: tenant.tier,
                totalItemCount: items.length,
            },
            rows: buildProcurementItemWorkspaceRows({
                categories: categoryRows,
                items: items.map((item) => ({
                    archivedAt: item.archivedAt ?? null,
                    categoryId: String(item.categoryId),
                    categoryName:
                        categoriesById.get(String(item.categoryId))?.name ??
                        "Unknown category",
...
export const createItem = mutation({
    ...
    handler: async (ctx, args) => {
        let mutationContext: Awaited<ReturnType<typeof loadItemMutationContext>> | null = null;
        try {
...
export const updateItem = mutation({
    ...
    handler: async (ctx, args) => {
        ...
        try {
            ...
            const nextRevision = getComparableProcurementItemRevision(item.revision) + 1;
            const now = Date.now();
        ...
        try {
            ...
            const categoriesByNormalizedName = new Map(
                categories
                    ...
                    .map((category) => [
                        normalizeProcurementItemName(category.name),
                        category,
                    ] as const),
            );
            const activeItemCountByCategoryId = new Map<string, number>();
            ...
            for (let index = 0; index < args.rows.length; index += 1) {
                ...
                const resolvedCategory =
                    categoriesByNormalizedName.get(normalizeProcurementItemName(categoryName)) ??
                    null;
                const name = normalizeProcurementItemDisplayName(
                    pickWorkbookValue(row, ["Item/Service Description", "Item Description", "Description"]),
                );
                const normalizedName = normalizeProcurementItemName(name);
                const duplicateKey = createProcurementItemImportDuplicateKey({
                    categoryId: resolvedCategory ? String(resolvedCategory._id) : categoryName,
                    normalizedName,
                });
                ...
                const price = normalizeUnitPrice(
                    pickWorkbookValue(row, ["Unit Price", "Price"]),
                ) ?? null;
                const minQuantity =
                    normalizeQuantityLimit(pickWorkbookValue(row, ["Min Quantity"])) ?? null;
                const maxQuantity =
                    normalizeQuantityLimit(pickWorkbookValue(row, ["Max Quantity"])) ?? null;
                const complianceInput = pickWorkbookValue(row, ["Compliance Flags", "Compliance"]);
                ...
                projectedCountByCategoryId.set(
                    String(resolvedCategory!._id),
                    (projectedCountByCategoryId.get(String(resolvedCategory!._id)) ?? 0) + 1,
                );
                createdCount += 1;
            }
...
function assertFreshItemRevision(item: Pick<ItemRecord, "revision">, expectedRevision: number) {
    if (getComparableProcurementItemRevision(item.revision) !== expectedRevision) {
        throw new ConvexError({
            code: "CONFLICT",
...
async function appendItemFailureAudit(
    ...
    const resolvedAuthContext = args.authContext ?? (await getAuthorizationContext(ctx));
    const actor =
...
function pickWorkbookValue(row: Record<string, unknown>, keys: readonly string[]): string {
