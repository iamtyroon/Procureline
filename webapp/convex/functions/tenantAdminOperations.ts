...
type TenantAdminContext = Awaited<ReturnType<typeof requireTenantRole>>;
...
function randomReader() {
    return { read(bytes: Uint8Array): void { crypto.getRandomValues(bytes); } };
}
...
async function encryptTotpSecret(secret: string): Promise<{ ciphertext: string; iv: string }> {
    const iv = crypto.getRandomValues(new Uint8Array(12));
    const encrypted = await crypto.subtle.encrypt(
        { iv, name: "AES-GCM" },
        await getTotpEncryptionKey(),
        new TextEncoder().encode(secret),
    );
    return { ciphertext: bytesToBase64(new Uint8Array(encrypted)), iv: bytesToBase64(iv) };
}

async function decryptTotpSecret(ciphertext: string, iv: string): Promise<string> {
    const decrypted = await crypto.subtle.decrypt(
        { iv: base64ToBytes(iv), name: "AES-GCM" },
        await getTotpEncryptionKey(),
...
function decodeBase32(secret: string): Uint8Array {
    ...
    for (const character of secret.replace(/=+$/, "").toUpperCase()) {
        ...
        if (index < 0) validationError("secret", "Invalid authenticator secret.");
...
async function computeTotp(secret: string, timestamp = Date.now()): Promise<string> {
...
async function getCurrentTenantAdmin(ctx: ReadCtx, auth: TenantAdminContext) {
    ...
    if (!member || !member.isActive) {
        throw new ConvexError({ code: "UNAUTHORIZED", message: "An active Tenant Admin membership is required." });
    }
    return member;
}
...
async function requireWritableTenant(ctx: MutationCtx, auth: TenantAdminContext): Promise<void> {
...
async function scheduleGenericEmail(ctx: MutationCtx, args: {
    heading: string;
    ...
    await ctx.scheduler.runAfter(0, internal.actions.email.queueTenantBackgroundEmail, {
...
async function requireTenantAdminVerificationSession(ctx: MutationCtx): Promise<TenantAdminContext> {
    const auth = await getAuthorizationContext(ctx);
    ...
    return auth as TenantAdminContext;
}

export const getInstitutionSettings = query({
    args: {},
    handler: async (ctx) => {
        ...
        if (!tenant) throw new ConvexError({ code: "NOT_FOUND", message: "Tenant not found." });
...
export const updateInstitutionSettings = mutation({
    args: {
        ...
        complianceTargets: v.object({ agpo: v.number(), localContent: v.number(), pwd: v.number() }),
        fiscalYearCustomFormat: v.optional(v.string()),
        fiscalYearDisplayFormat: v.union(v.literal("FY2025-26"), v.literal("2025/2026"), v.literal("custom")),
        ...
        timeZone: v.string(),
        confirmActiveCycleChange: v.boolean(),
    },
    handler: async (ctx, args) => {
        ...
        const tenant = await ctx.db.get(auth.tenantId);
        if (!tenant) throw new ConvexError({ code: "NOT_FOUND", message: "Tenant not found." });
        if (args.fiscalYearStartMonth !== INSTITUTIONAL_FISCAL_YEAR_START_MONTH) {
            validationError("fiscalYearStartMonth", "The institutional fiscal year is fixed from 1 July through 30 June.");
        }
        try {
            new Intl.DateTimeFormat("en-US", { timeZone: args.timeZone.trim() }).format(new Date());
        } catch {
            validationError("timeZone", "Select a valid IANA timezone.");
        }
        const complianceErrors = validateComplianceTargets(args.complianceTargets);
        if (complianceErrors.length > 0) validationError("complianceTargets", complianceErrors.join(" "));
        ...
        const domains = Array.from(new Set(args.allowedEmailDomains.map(normalizeAllowedEmailDomain)));
        if (domains.some((domain) => domain === null)) validationError("allowedEmailDomains", "Enter valid email domains only.");
        ...
        const removedDomains = (previous?.allowedEmailDomains ?? []).filter((domain) => !(domains as string[]).includes(domain));
        ...
        const recordId = await ctx.db.insert("tenantSettingsVersions", {
            allowedEmailDomains: domains as string[],
...
export const generateInstitutionLogoUploadUrl = mutation({
    args: {},
    handler: async (ctx) => {
        const auth = await requireTenantRole(ctx, ["tenant_admin"]);
        await requireWritableTenant(ctx, auth);
...
export const saveInstitutionLogo = mutation({
    ...
    handler: async (ctx, args) => {
        ...
        if (!logoUrl) validationError("logo", "Uploaded logo is unavailable.");
...
export const updateCurrentProfile = mutation({
    args: { name: v.string(), phone: v.string(), image: v.optional(v.string()) },
    handler: async (ctx, args) => {
        const auth = await requireTenantRole(ctx, ["tenant_admin"]);
        await requireWritableTenant(ctx, auth);
        const actor = await getCurrentTenantAdmin(ctx, auth);
        if (args.name.trim().length < 2) validationError("name", "Name is required.");
...
export const requestVerifiedEmailChange = mutation({
    args: {
    ...
    handler: async (ctx, args) => {
        ...
        if (!target || target.tenantId !== auth.tenantId || (args.purpose === "po_email" && target.role !== "procurement_officer") || (args.purpose === "tenant_admin_email" && target.userId !== auth.userId)) {
            throw new ConvexError({ code: "NOT_FOUND", message: "Email change target not found." });
        }
        const requestedEmail = args.requestedEmail.trim().toLowerCase();
        if (!/^[^@\s]+@[^@\s]+\.[^@\s]+$/.test(requestedEmail)) validationError("requestedEmail", "Enter a valid email.");
        const token = generateRandomString(randomReader(), "ABCDEFGHJKLMNPQRSTUVWXYZabcdefghijkmnopqrstuvwxyz23456789", 48);
        ...
        if (!queued) {
            throw new ConvexError({ code: "EMAIL_UNAVAILABLE", message: "The verification email could not be queued." });
        }
...
export const confirmVerifiedEmailChange = mutation({
    ...
    handler: async (ctx, args) => {
        ...
        if (!change || change.tenantId !== auth.tenantId || change.status !== "pending" || change.expiresAt <= Date.now()) validationError("verificationToken", "Email verification request is expired or unavailable.");
...
export const getPoLifecycleContext = query({
    args: { tenantUserId: v.optional(v.id("tenantUsers")) },
    handler: async (ctx, args) => {
        const auth = await requireTenantRole(ctx, ["tenant_admin"]);
        if (!args.tenantUserId) return null;
        const member = await ctx.db.get(args.tenantUserId);
        if (!member || member.tenantId !== auth.tenantId || member.role !== "procurement_officer") return null;
...
export const setProcurementOfficerActive = mutation({
    args: { active: v.boolean(), confirmImpact: v.boolean(), tenantUserId: v.id("tenantUsers") },
    handler: async (ctx, args) => {
        const auth = await requireTenantRole(ctx, ["tenant_admin"]);
        await requireWritableTenant(ctx, auth);
        const actor = await getCurrentTenantAdmin(ctx, auth);
        ...
        if (!member || member.tenantId !== auth.tenantId || member.role !== "procurement_officer") {
            throw new ConvexError({ code: "NOT_FOUND", message: "Procurement Officer membership not found." });
        }
        if (!args.active) {
            ...
            if (member.isActive && activePos.length <= 1) validationError("tenantUserId", "Cannot deactivate. At least one active PO required.");
            const departments = await ctx.db.query("departments").withIndex("by_procurementOfficerTenantUserId", (q) => q.eq("procurementOfficerTenantUserId", member._id)).collect();
            ...
            if (activeCycle && !args.confirmImpact) validationError("confirmImpact", "This PO manages departments in an active submission cycle. Confirm deactivation to continue.");
        }
...
export const listProcurementOfficerActivity = query({
    args: { tenantUserId: v.id("tenantUsers"), dateFrom: v.optional(v.number()), dateTo: v.optional(v.number()), action: v.optional(v.string()), entityType: v.optional(v.string()), page: v.optional(v.number()) },
    handler: async (ctx, args) => {
        const auth = await requireTenantRole(ctx, ["tenant_admin"]);
        const member = await ctx.db.get(args.tenantUserId);
        if (!member || member.tenantId !== auth.tenantId || member.role !== "procurement_officer") return { items: [], page: 1, totalPages: 1 };
...
export const unlockProcurementOfficer = mutation({
    args: { tenantUserId: v.id("tenantUsers") },
    handler: async (ctx, args) => {
        const auth = await requireTenantRole(ctx, ["tenant_admin"]);
        await requireWritableTenant(ctx, auth);
        const member = await ctx.db.get(args.tenantUserId);
        if (!member || member.tenantId !== auth.tenantId || member.role !== "procurement_officer") throw new ConvexError({ code: "NOT_FOUND", message: "Procurement Officer not found." });
...
export const getBillingWorkspace = query({
    args: {},
    handler: async (ctx) => {
        const auth = await requireTenantRole(ctx, ["tenant_admin"]);
        ...
        const numeric = (value: number | string | undefined): number | null => typeof value === "number" ? value : null;
...
export const requestSubscriptionChange = mutation({
    ...
    handler: async (ctx, args) => {
        ...
        const tenant = await ctx.db.get(auth.tenantId);
        if (!tenant) throw new ConvexError({ code: "NOT_FOUND", message: "Tenant not found." });
        const order: TenantTier[] = ["free", "starter", "professional", "enterprise"];
        if (args.toTier === tenant.tier) validationError("toTier", "Select a different tier.");
        ...
        const blockers = isDowngrade ? buildDowngradeBlockers([{ key: "departments", label: "Departments", current: departments.length, limit: typeof tier?.limits.departments === "number" ? tier.limits.departments : null }]) : [];
...
export const getSecurityWorkspace = query({
    args: {},
    handler: async (ctx) => {
        const auth = await requireTenantRole(ctx, ["tenant_admin"]);
        const actor = await getCurrentTenantAdmin(ctx, auth);
...
export const beginTwoFactorEnrollment = mutation({
    args: {},
    handler: async (ctx) => {
        const auth = await requireTenantRole(ctx, ["tenant_admin"]);
        await requireWritableTenant(ctx, auth);
        const actor = await getCurrentTenantAdmin(ctx, auth);
        const secret = generateRandomString(randomReader(), "ABCDEFGHJKLMNPQRSTUVWXYZ23456789", 24);
        ...
        if (existing) await ctx.db.patch(existing._id, { totpEnrollmentPendingHash: pending, updatedAt: Date.now() });
        else await ctx.db.insert("tenantAdminSecurityStates", { createdAt: Date.now(), failedLoginAttempts: 0, isTwoFactorEnrolled: false, recoveryCodes: [], tenantId: auth.tenantId, tenantUserId: actor._id, totpEnrollmentPendingHash: pending, updatedAt: Date.now(), userId: auth.userId });
        return { qrRepresentation: `otpauth://totp/Procureline:${auth.userId}?secret=${secret}&issuer=Procureline`, secret };
    },
});

export const confirmTwoFactorEnrollment = mutation({
    ...
    handler: async (ctx, args) => {
        ...
        if (!state?.totpEnrollmentPendingHash || await hashSecret(args.secret) !== state.totpEnrollmentPendingHash) validationError("secret", "Authenticator enrollment is unavailable.");
        const expectedCodes = await Promise.all([-30_000, 0, 30_000].map((offset) => computeTotp(args.secret, Date.now() + offset)));
        if (!expectedCodes.includes(args.code.trim())) validationError("code", "Invalid authenticator confirmation code.");
        const codes = Array.from({ length: 10 }, () => generateRandomString(randomReader(), "ABCDEFGHJKLMNPQRSTUVWXYZ23456789", 10));
...
export const acknowledgeTwoFactorRecoveryCodes = mutation({
    args: {},
    handler: async (ctx) => {
        const auth = await requireTenantRole(ctx, ["tenant_admin"]);
        await requireWritableTenant(ctx, auth);
...
export const verifyCurrentTwoFactorCode = mutation({
    ...
    handler: async (ctx, args) => {
        ...
        if (!expectedCodes.includes(args.code.trim())) {
            ...
            await ctx.db.patch(state._id, { failedLoginAttempts, lockedUntil: computeLockoutUntil({ failedAttempts: failedLoginAttempts, now: Date.now() }) ?? state.lockedUntil, updatedAt: Date.now() });
            validationError("code", "Invalid authenticator verification code.");
        }
        const currentSession = await loadCurrentSessionDocuments(ctx);
        if (!currentSession) {
            throw new ConvexError({ code: "UNAUTHORIZED", message: "The current session cannot be verified." });
        }
        if (currentSession.metadata) {
            await ctx.db.patch(currentSession.metadata._id, { tenantAdminAuthStage: "verified", tenantAdminVerifiedAt: Date.now() });
        } else {
...
export const verifyCurrentRecoveryCode = mutation({
    args: { code: v.string() },
    handler: async (ctx, args) => {
        const auth = await requireTenantAdminVerificationSession(ctx);
        const actor = await getCurrentTenantAdmin(ctx, auth);
        ...
        if (!state?.isTwoFactorEnrolled || recoveryIndex < 0) validationError("code", "Invalid or previously used recovery code.");
        const recoveryCodes = state.recoveryCodes.map((code, index) => index === recoveryIndex ? { ...code, consumedAt: Date.now() } : code);
        const currentSession = await loadCurrentSessionDocuments(ctx);
        if (!currentSession) throw new ConvexError({ code: "UNAUTHORIZED", message: "The current session cannot be verified." });
...
export const revokeOtherSession = mutation({
    args: { sessionMetadataId: v.id("sessionMetadata") },
    handler: async (ctx, args) => {
        const auth = await requireTenantRole(ctx, ["tenant_admin"]);
        await requireWritableTenant(ctx, auth);
        const target = await ctx.db.get(args.sessionMetadataId);
        if (!target || target.userId !== auth.userId) throw new ConvexError({ code: "NOT_FOUND", message: "Session not found." });
        const currentSession = await loadCurrentSessionDocuments(ctx);
...
export const getAccountLifecycle = query({
    args: {},
    handler: async (ctx) => {
        const auth = await requireTenantRole(ctx, ["tenant_admin"]);
        const actor = await getCurrentTenantAdmin(ctx, auth);
...
export const initiateAdminTransfer = mutation({
    args: { recipientTenantUserId: v.id("tenantUsers") },
    handler: async (ctx, args) => {
        const auth = await requireTenantRole(ctx, ["tenant_admin"]);
        await requireWritableTenant(ctx, auth);
        const actor = await getCurrentTenantAdmin(ctx, auth);
        const recipient = await ctx.db.get(args.recipientTenantUserId);
        if (!recipient || recipient.tenantId !== auth.tenantId || !recipient.isActive || recipient._id === actor._id) validationError("recipientTenantUserId", "Select an eligible active tenant member.");
        const token = generateRandomString(randomReader(), "ABCDEFGHJKLMNPQRSTUVWXYZabcdefghijkmnopqrstuvwxyz23456789", 48);
        ...
        if (!queued) throw new ConvexError({ code: "EMAIL_UNAVAILABLE", message: "The transfer acceptance email could not be queued." });
...
export const acceptAdminTransfer = mutation({
    args: { acceptanceToken: v.string() },
    handler: async (ctx, args) => {
        ...
        if (!transfer || transfer.tenantId !== auth.tenantId || transfer.status !== "pending_acceptance" || transfer.expiresAt <= Date.now()) validationError("acceptanceToken", "Transfer request is unavailable or expired.");
        const recipient = await ctx.db.get(transfer.recipientTenantUserId);
        if (!recipient || recipient.userId !== auth.userId || !recipient.isActive) validationError("acceptanceToken", "Only the nominated active recipient may accept.");
        ...
        if (recipient.role !== "tenant_admin") await ctx.db.insert("tenantUsers", { isActive: true, role: "tenant_admin", tenantId: auth.tenantId, userId: recipient.userId });
        if (admins.length > 0) await ctx.db.patch(transfer.initiatedByTenantUserId, { isActive: false, deactivatedAt: Date.now() });
...
export const requestAccountDeletion = mutation({
    ...
    handler: async (ctx) => {
        ...
        const actor = await getCurrentTenantAdmin(ctx, auth);
        const admins = await ctx.db.query("tenantUsers").withIndex("by_tenantId", (q) => q.eq("tenantId", auth.tenantId)).filter((q) => q.eq(q.field("role"), "tenant_admin")).filter((q) => q.eq(q.field("isActive"), true)).collect();
        if (admins.length <= 1) validationError("account", "Transfer Tenant Admin responsibility before deleting this account.");
...
export const restoreDeletedTenantAdminAccount = mutation({
    args: {},
    handler: async (ctx) => {
        const userId = await getAuthUserId(ctx);
        if (!userId) throw new ConvexError({ code: "UNAUTHORIZED", message: "Sign in to restore this account." });
        ...
        if (!membership) validationError("account", "No recoverable Tenant Admin account is available.");
...
export const runTenantSubscriptionMaintenance = internalMutation({
    args: {},
    handler: async (ctx) => {
        const now = Date.now();
        ...
        for (const tenant of tenants) {
            ...
            const reminderCandidates: Array<{ key: string; type: "deletion_review" | "grace_daily" | "renewal_30" | "renewal_7"; title: string; message: string; priority: "critical" | "high" }> = [];
            ...
            if (tenant.subscriptionNextBillingDate) {
                ...
                if (days === 30 || days === 7) reminderCandidates.push({ key: `billing:renewal:${tenant._id}:${days}:${tenant.subscriptionNextBillingDate}`, type: days === 30 ? "renewal_30" : "renewal_7", title: `Renewal due in ${days} days`, message: `Subscription renewal is due ${new Date(tenant.subscriptionNextBillingDate).toLocaleDateString()}.`, priority: "high" });
            }
