    ...
    handler: async (ctx) => {
        ...
        for (const plan of plans) {
            ...
            for (const categoryId of Array.from(referencedCategoryIds)) {
                const existing = planStatusesByCategoryId.get(categoryId) ?? new Set<CategoryPlanStatus>();
        ...
        const summary = buildCategoryWorkspaceSummary({
            categories: categories.map((category) => ({
                ...
                planStatuses: Array.from(
                    planStatusesByCategoryId.get(String(category._id)) ?? new Set<CategoryPlanStatus>(),
                ),
        ...
        return {
            meta: {
                ...
                nextSortOrder:
                    Math.max(
                        0,
                        ...summary.rows.map((row) => row.sortOrder),
                    ) + 1,
...
export const createCategory = mutation({
    ...
    handler: async (ctx, args) => {
        let mutationContext: Awaited<ReturnType<typeof loadCategoryMutationContext>> | null =
            null;
        try {
            ...
            const existingCategories = await ctx.db
                ...
                .collect();
            const nextSortOrder =
                Math.max(0, ...existingCategories.map((category) => category.sortOrder)) + 1;
            const now = Date.now();
    ...
    handler: async (ctx, args) => {
        ...
        try {
            ...
            const categoriesById = new Map(
                activeCategories.map((category) => [String(category._id), category] as const),
            );
            const expectedRevisionById = new Map(
                args.expectedRevisions.map((entry) => [entry.categoryId, entry.revision] as const),
            );
            ...
            for (let index = 0; index < args.rows.length; index += 1) {
                ...
                const record =
                    row && typeof row === "object" && !Array.isArray(row)
                        ...
                        : {};
                const name = normalizePlainTextValue(record[CATEGORY_IMPORT_COLUMNS[0]]);
                const description = normalizePlainTextValue(record[CATEGORY_IMPORT_COLUMNS[1]]);
                const colorInput = normalizePlainTextValue(record[CATEGORY_IMPORT_COLUMNS[2]]);
...
async function assertCategoryTierCapacity(
    ...
    const limits: Record<Exclude<Doc<"tenants">["tier"], "enterprise">, number> = {
        free: 20,
        professional: 200,
        starter: 60,
    };
