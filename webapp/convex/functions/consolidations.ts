...
async function readTenantUserActorSummary(
    ctx: DataCtx,
    tenantUserId: Id<"tenantUsers"> | null | undefined,
): Promise<TenantUserActorSummary | null> {
    if (!tenantUserId) {
        return null;
    }
...
async function loadConsolidationForFiscalYear(ctx: DataCtx, args: {
    fiscalYear: string;
    tenantId: Id<"tenants">;
}) {
    return ctx.db
        ...
        .withIndex("by_tenantId_fiscalYear", (q) =>
            q.eq("tenantId", args.tenantId).eq("fiscalYear", args.fiscalYear),
        )
        .first();
}

async function loadSnapshotForConsolidation(ctx: DataCtx, args: {
    consolidationId: Id<"consolidations">;
}) {
    return ctx.db
        .query("consolidationSnapshots")
        .withIndex("by_consolidationId", (q) =>
            q.eq("consolidationId", args.consolidationId),
        )
        .order("desc")
        .first();
}
...
function mapSnapshot(snapshot: Doc<"consolidationSnapshots"> | null) {
    if (!snapshot) {
        return null;
    }
...
function createCompactConsolidationWorkspaceRecord(args: {
...
}): BlocklyWorkspaceRecord | undefined {
    if (!args.workspaceState) {
        return undefined;
    }

    const normalizedRecord = normalizeBlocklyWorkspaceRecord(args.workspaceState);
    const workspaceJson = normalizeBlocklyWorkspaceJson(normalizedRecord.workspaceJson);
    ...
    return createPersistedBlocklyWorkspaceRecord(
        createBlocklyWorkspaceRecord({
...
function getSelectedSourceDepartmentIds(draftData: unknown): string[] {
...
export const getProcurementOfficerConsolidationWorkspace = query({
    ...
    handler: async (ctx, args) => {
        ...
        const [finalizedByActor, snapshotCapturedByActor] = await Promise.all([
            readTenantUserActorSummary(
                ctx,
                consolidation?.finalizedByTenantUserId ?? null,
            ),
    ...
    handler: async (ctx, args) => {
        ...
        if (existingConsolidation?.status === "finalized") {
            throw new ConvexError({
                ...
                message:
                    "Use Edit Draft before saving changes to a finalized consolidation.",
            });
        }
...
export const finalizeProcurementOfficerConsolidation = mutation({
    args: {
    ...
    handler: async (ctx, args) => {
        ...
        if (consolidation.revision !== args.expectedRevision) {
            throw new ConvexError({
                ...
                message:
                    "This consolidation draft changed in another tab or device. Refresh before finalizing.",
            });
        }

        const selectedSourceDepartmentIds = getSelectedSourceDepartmentIds(
            consolidation.draftData,
        );
        if (selectedSourceDepartmentIds.length === 0) {
            throw new ConvexError({
                code: "VALIDATION_FAILED",
                message:
                    "Connect at least one approved source department before finalizing.",
            });
        }
        assertSelectedSourcesAreStillApproved({
            readyDepartmentIds: base.readiness.readyDepartmentIds,
        ...
        const selectedSourcePlanIds = base.readiness.readyDepartments
            .filter((department) =>
                selectedSourceDepartmentIds.includes(department.departmentId),
            )
            .map((department) => department.planId);
        const snapshotValues = extractConsolidationFinalizationSnapshotValues(
            consolidation.workspaceState,
        );
        const now = Date.now();
        const snapshotId = await ctx.db.insert("consolidationSnapshots", {
            ...
            schemaVersion: CONSOLIDATION_SNAPSHOT_SCHEMA_VERSION,
            selectedSourceDepartmentIds,
        ...
        await appendAuditLogRequired(
            ...
            buildConsolidationAuditEntry({
                ...
                actorUserId: authContext.userId,
                consolidationId: consolidation._id,
                event: AUDIT_EVENT_NAMES.consolidationFinalized,
                fiscalYear: base.readiness.selectedFiscalYear,
                metadata: {
...
export const reopenProcurementOfficerConsolidationForEditing = mutation({
    args: {
        expectedRevision: v.number(),
        fiscalYear: v.string(),
    },
    ...
    handler: async (ctx, args) => {
        ...
        if (consolidation.revision !== args.expectedRevision) {
            throw new ConvexError({
                ...
                currentRevision: consolidation.revision,
                message:
                    "This consolidation changed in another tab or device. Refresh before editing.",
            });
        }
        ...
        await appendAuditLogRequired(
            ...
            buildConsolidationAuditEntry({
                ...
                actorUserId: authContext.userId,
                consolidationId: consolidation._id,
                event: AUDIT_EVENT_NAMES.consolidationReopened,
                fiscalYear: base.readiness.selectedFiscalYear,
                metadata: {
