...
export const PROCUREMENT_ITEM_NAME_REQUIRED_MESSAGE = "Item description is required";
export const PROCUREMENT_ITEM_CATEGORY_REQUIRED_MESSAGE = "Category is required";
...
export const PROCUREMENT_ITEM_IMPORT_BLANK_CATEGORY_MESSAGE = "Category is required";
...
export type ProcurementItemTier = "enterprise" | "free" | "professional" | "starter";
...
export interface ProcurementItemEditorCategoryOption
...
export function normalizeProcurementMethod(
    ...
    const normalized = normalizePlainText(input ?? "");
    return (
        PROCUREMENT_ITEM_PROCUREMENT_METHODS.find((method) => method === normalized) ??
        undefined
    );
}
    ...
    return Array.from(
        new Set(
            rawValues
                ...
                .filter(
                    (value): value is ProcurementItemComplianceFlag =>
...
export function buildProcurementItemImportLimitState(args: {
    ...
    return {
        rowLimit:
            args.tier === "enterprise" ? null : PROCUREMENT_ITEM_IMPORT_LIMITS[args.tier],
        tier: args.tier,
        tierLabel: PROCUREMENT_ITEM_TIER_LABELS[args.tier],
    };
}
    ...
    return [...args.items]
        ...
        .map((item) => {
            ...
            return {
                ...item,
                archivedLabel:
                    item.isActive || item.archivedAt === null ? null : "Archived",
...
export function parseStoredItemDraft(
    ...
    try {
        ...
        const maxQuantityInput =
            typeof record.maxQuantity === "number" || typeof record.maxQuantity === "string"
                ? record.maxQuantity
                : null;
        const minQuantityInput =
            typeof record.minQuantity === "number" || typeof record.minQuantity === "string"
                ? record.minQuantity
                : null;
...
export function normalizeImportedProcurementItemUnit(
    value: string,
...
export function isProcurementItemTierLimitMessage(message: string | null): boolean {
    return message ? PROCUREMENT_ITEM_TIER_LIMIT_PATTERN.test(message) : false;
}

export function isProcurementItemCrudAuthorizationError(error: unknown): boolean {
    const message =
        error instanceof Error && error.message.trim().length > 0
            ? error.message.trim()
            : null;
