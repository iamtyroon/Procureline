...
interface ProcurementItemBackendIndexBuilder {
    eq(field: string, value: unknown): ProcurementItemBackendIndexBuilder;
}

export interface ProcurementItemBackendQueryCtx {
    db: {
        query(tableName: "procurementItems"): {
            withIndex(
                indexName: "by_tenantId_categoryId",
                build: (builder: ProcurementItemBackendIndexBuilder) => unknown,
            ): {
                collect(): Promise<ProcurementItemBackendRecord[]>;
            };
        };
    };
}
