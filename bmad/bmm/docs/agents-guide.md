...
### Specialized Task Sub-Agents

BMM includes specialized sub-agents that are invoked programmatically by main agents during workflow execution. These sub-agents operate behind the scenes to provide specialized capabilities.

#### Schema Surveyor (bmm-schema-surveyor)

**Purpose:** Runtime database intelligence agent that provides accurate, current Supabase schema information to prevent hallucination-driven errors during database-related development.

**Invoked By:** DEV agent during database-related story implementation via the Task tool

**Key Features:**
- **Session-cached schema data:** 85% query time reduction through intelligent caching
- **Three-part output:** Raw MCP results (proof of work) + Human summary + Structured JSON
- **Fuzzy table matching:** Handles plural/singular, word order variations, compound words
- **Domain-based organization:** Groups tables by logical domains (User Management, NDT Core, etc.)
- **Auto-refresh resilience:** Detects schema drift and automatically refreshes cache

**Critical Constraints:**
- **MANDATORY MCP usage:** Never assumes or guesses - all data retrieved from live database
- **Proof of work:** Displays actual MCP query results before summarizing
- **Zero hallucination policy:** Bans phrases like "typically", "usually", "common pattern"

**Project Configuration:** NDT Inspection Toolkit (Project ID: `xsrimymebzinenvzyblz`)

**User Interaction:** Users rarely interact with Schema Surveyor directly - it's automatically invoked by the DEV agent when database schema knowledge is required for story implementation.

**Technical Details:**
- Location: `.claude/agents/bmad-analysis/schema-surveyor.md`
- Version: 2.0 (Overhauled 2025-11-06)
- Output: Comprehensive schema reports with security policies, performance notes, and relationship mapping
