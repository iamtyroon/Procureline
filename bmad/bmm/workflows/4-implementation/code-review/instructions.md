...
Your input:</ask>
    ...
    <action>Extract {{epic_letter}}, {{epic_num}}, and {{story_num}} from filename:
      - Pattern: story-{epic_letter}.{story_num}-*.md (e.g., story-b.6-customers.md)
      - OR from story key: {epic_letter}-{story_num}-* (e.g., "b-6-customers")
      - Set {{epic_letter}} = letter identifier (e.g., "b" for Epic B)
      - Set {{epic_num}} = numeric form if needed (e.g., "6")
      - Set {{story_num}} = story number within epic
    </action>
  ...
  <step n="2" goal="Resolve story context file and architecture docs">
    <action>Locate story context file: Under Dev Agent Record → Context Reference, read referenced path(s). If missing, search {{output_folder}} for files matching pattern "story-{{epic_letter}}.{{story_num}}*.context.xml" and use the most recent.</action>
    ...
    <note>Tech-spec loading is now handled in dedicated Step 2.5 below with intelligent sharding detection</note>
  </step>

  <step n="2.5" goal="Intelligently locate and load Epic Tech Spec (whole or sharded)">
    <critical>SMART TECH-SPEC DISCOVERY - Detect whether tech-spec is whole or sharded, load appropriately</critical>

    <action>Extract epic identifier from story:
      - Parse story filename pattern: "story-{epic_letter}.{story_num}-*.md" (e.g., "story-b.6-customers.md")
      - OR parse story key pattern: "{epic_letter}-{story_num}-*" (e.g., "b-6-customers")
      - Extract {{epic_letter}} (e.g., "b" from Epic B)
    </action>

    <action>Resolve epic folder path:
      1. Search {{tech_spec_search_root}} for directories matching pattern: "Epic-{{epic_letter|uppercase}}-*"
         - Example: For epic_letter="b", search for "Epic-B-*"
         - Expected match: "Epic-B-Web-Review-PDF-Generation"
      2. If multiple matches found, use the first one (should be unique)
      3. If no match found, set {{epic_folder_path}} = null and record WARNING
    </action>

    <action if="{{epic_folder_path}} found">Try to locate tech-spec in order of preference:

      **Option 1: Check for WHOLE tech-spec file**
      - Search path: {{epic_folder_path}}/tech-spec-epic-{{epic_letter|lowercase}}.md
      - Example: /docs/05-Epics-Stories/Epic-B-Web-Review-PDF-Generation/tech-spec-epic-b.md
      - If found: Read complete file, set {{tech_spec_mode}} = "whole", set {{tech_spec_content}} = file content

      **Option 2: Check for SHARDED tech-spec**
      - If whole file not found, search for: {{epic_folder_path}}/tech-spec-epic-{{epic_letter|lowercase}}/index.md
      - Example: /docs/05-Epics-Stories/Epic-B-Web-Review-PDF-Generation/tech-spec-epic-b/index.md
      - If index.md found:
        1. Read index.md completely
        2. Extract list of section files from Table of Contents links (e.g., "./overview.md", "./detailed-design.md")
        3. Read ALL section files in the sharded folder
        4. Combine content: index.md + all section files
        5. Set {{tech_spec_mode}} = "sharded"
        6. Set {{tech_spec_content}} = combined content

      **Option 3: Tech-spec not found**
      - If neither whole nor sharded version found:
        - Set {{tech_spec_found}} = false
        - Record WARNING in review notes: "No Tech Spec found for Epic {{epic_letter|uppercase}}"
        - Continue review but flag this as a documentation gap
    </action>

    <action if="{{tech_spec_found}} == true">
      - Log tech-spec discovery: "Tech Spec loaded: {{tech_spec_mode}} mode (Epic {{epic_letter|uppercase}})"
      - Use {{tech_spec_content}} for cross-checking in Step 4C (architecture alignment validation)
    </action>

    <action if="{{epic_folder_path}} == null">
      - Record WARNING: "Epic folder not found for Epic {{epic_letter|uppercase}} - cannot locate tech-spec"
      - Set {{tech_spec_found}} = false
      - Continue review with limited context
    </action>
  </step>
